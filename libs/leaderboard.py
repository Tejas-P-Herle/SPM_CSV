...
def getLeaderboard(grade, section, exam='', ranks=10):
    totals = {}
    leaderboard = []
    stdntsData = db.get_class_uid(grade, section)
...
def getSubjLeaderboard(grade, section, exam, subj, ranks=10):
    totals = {}
    leaderboard = []
    stdntData = db.get_class_uid(grade, section)
...
def getExmTotal(uid, exam=''):
    if not exam:
        marks = 0
        for exm in exams:
            marks += sum(db.get_marks(uid, exm))
        return marks
    return sum(db.get_marks(uid, exam))
