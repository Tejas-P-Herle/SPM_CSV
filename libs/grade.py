...
from libs.leaderboard import getLeaderboard, getExmTotal, getExmBest, getSubjBest
...
class Grade:
    def __init__(self, grade, section):
        ...
        Calculations like getting the class best, class average etc which would
    ...
    def getStdntPosPercent(self, uid, exm):
        ...
        _, stud_pct = self.getClassPercent(exm, names=False)
        percents = []
        averages = []
        top = []
        bottom = []
        for i in range(len(stud_pct[0])):
            percents.append([stud[i] for stud in stud_pct])
        for exam in percents:
            averages.append(sum(exam) / len(exam))
            top.append(max(exam))
            bottom.append(min(exam))
        stud_pos_percent = [top, self.getExmPercent(uid, exm), averages, bottom]
        print_('Grade method: getStdntPosPercent-Complete')
        return stud_pos_percent
    ...
    def getClassPercentImpr(self, exm):
        ...
        if exm:
            ...
            _, prevPercents = self.getClassPercent(exams[index], names=False)
            percentImprs = []
    ...
    def getClassPercent(self, exm, names=True):
        print_r('Grade method: getClassPercent')
        stud_names = None
        if names:
            stdnts = db.get_students(self.grade, self.section)
            stud_names = [s[1] for s in stdnts]
        else:
            stdnts = db.get_class_uid(self.grade, self.section)
        percents = [self.getExmPercent(s[0], exm) for s in stdnts]
        print_r('Grade method: getClassPercent-Complete')
        return stud_names, percents
    ...
    def getExmPercent(self, uid, exam):
        ...
        if exam:
            ...
            return self.calcPercent(db.get_marks(uid, exam), sa)
        percents = []
        for exam in exams:
            maxMarks = 300 if exam[0] == 'F' else 625
            total = sum(db.get_marks(uid, exam))
    ...
    def getExmPertile(self, uid, exam):
        ...
        if exam:
            return self.calcPertile(db.get_marks(uid, exam), exam)
        pertiles = []
        for exam in exams:
    ...
    def getStats(self, ranks, exam=''):
        ...
        for uid in uids:
            ...
            ttls.append(round(getExmTotal(u, exam), 2))
